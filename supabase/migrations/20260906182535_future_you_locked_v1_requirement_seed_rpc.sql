-- Internal, idempotent requirement seeding for one I3 intake instance.
-- The caller is the protected Edge Function; anonymous and normal client roles
-- cannot execute this helper.
...
begin
  if jsonb_typeof(p_requirements) <> 'array' then
    raise exception 'future_you_requirements_array_required';
  end if;
  ...
  insert into public.intake_requirements (
    intake_instance_id,
    user_id,
    requirement_key,
    priority,
  ...
  select
    p_intake_instance_id,
    p_user_id,
    r.requirement_key,
    r.priority,
    'active',
    'missing',
    concat('Locked I2 requirement; decision area: ', r.decision_area)
  from jsonb_to_recordset(p_requirements) as r(
    requirement_key text,
    priority text,
    decision_area text
  )
  where r.requirement_key is not null
    and r.priority in ('essential_now', 'conditional', 'optional_optimization', 'learn_later')
