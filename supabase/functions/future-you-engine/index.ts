...
// --- inlined from intake-dynamic.ts (dashboard editor only supports one file) ---
const LIVE_SEMANTIC_INTAKE_VERSION = "future-you-semantic-intake-v6.0-dynamic";
const QUICK_START_MAX_QUESTIONS = 5;
const FULL_PLAN_SAFETY_CAP = 18;
const MAX_INTAKE_INPUT_TOKENS = 16_000;
const MAX_INTAKE_OUTPUT_TOKENS = 4_000;

function intakeCapForLevel(level: string): number {
...
const liveIntakeInstructions = `You own the next Future You intake decision for one literal goal.
...
function addIntakeUsage(current: Partial<IntakeUsage>, addition: Partial<IntakeUsage> | null | undefined): IntakeUsage {
  return {
...
function assessSimulatedAnswerQuality(question: any, answer: any): Record<string, any> {
...
function buildEvidenceItems(goal: string, priorTurns: any[]): EvidenceItem[] {
...
function assertLiveInputTokenCeiling(inputTokens: number): number {
...
function validateAndBuildLiveResult(args: {
  goal: string;
  output: any;
  previousState: any;
