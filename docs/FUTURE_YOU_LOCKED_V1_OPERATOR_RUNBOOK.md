...
All derivation calls use the Responses API with `store: false`, enforced centrally even if a future derivation requests otherwise, plus a bounded request deadline, JSON-object output validation, and local Locked v1 contract validation. The only model-side user identifier is a SHA-256 digest; no email or raw Supabase user ID is sent for that purpose. Model/provider error bodies are not written to function logs. Unit coverage exercises completed responses, incomplete responses, malformed JSON, the standard output-content fallback, and provider-error redaction.
